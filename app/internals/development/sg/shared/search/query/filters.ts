...
import { languageCompletion } from './languageFilter.ts'
import { selectorCompletion } from './selectFilter.ts'
import type { Filter, Literal } from './token.ts'
...
export const FILTERS: Record<NegatableFilter, NegatableFilterDefinition> | any &
    ...
    [FilterType.author]: {
        negatable: true,
        description: (negated: any) => `${negated ? 'Exclude' : 'Include only'} commits or diffs authored by a user.`,
    ...
    [FilterType.lang]: {
        alias: 'l',
        discreteValues: (value: any) => languageCompletion(value).map(toCompletionItem),
        negatable: true,
        description: (negated: any) => `${negated ? 'Exclude' : 'Include only'} results from the given language`,
    },
    [FilterType.message]: {
        alias: 'm',
        negatable: true,
    ...
    [FilterType.patterntype]: {
        discreteValues: () => {
            ...
            if (typeof window === 'undefined' || (window as any).context?.experimentalFeatures?.structuralSearch === 'enabled') {
        ...
        description: `The pattern type (keyword, literal, regexp, standard${
            typeof window === 'undefined' || (window as any).context?.experimentalFeatures?.structuralSearch === 'enabled'
                ? ', structural'
                : ''
        }) in use`,
    ...
    [FilterType.repo]: {
        alias: 'r',
        negatable: true,
        discreteValues: (_value: any, isSourcegraphDotCom: any) => [
    ...
    [FilterType.select]: {
        discreteValues: (value: any) => selectorCompletion(value).map(value => ({ label: value })),
