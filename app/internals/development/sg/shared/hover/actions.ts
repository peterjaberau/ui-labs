...
import { ContributableMenu, type TextDocumentPositionParameters } from '~/internals/development/sg/client-api'
import { type HoveredToken, LOADER_DELAY, type MaybeLoadingResult, emitLoading } from '~/internals/development/sg/codeintellify'
import {
    asError,
    compatNavigate,
    type ErrorLike,
    ...
    isExternalLink,
    logger,
} from '~/internals/development/sg/common'
import type { Location } from '~/internals/development/sg/extension-api-types'
...
import { wrapRemoteObservable } from '../api/client/api/common.ts'
import type { FlatExtensionHostAPI } from '../api/contract.ts'
import type { WorkspaceRootWithMetadata } from '../api/extension/extensionHostApi.ts'
import { syncRemoteSubscription } from '../api/util.ts'
import { resolveRawRepoName } from '../backend/repo.ts'
import { languageSpecs } from '../codeintel/legacy-extensions/language-specs/languages.ts'
import { getContributedActionItems } from '../contributions/contributions.ts'
import type { Controller, ExtensionsControllerProps } from '../extensions/controller.ts'
import type { PlatformContext, PlatformContextProps, URLToFileContext } from '../platform/context.ts'
import { makeRepoGitURI, parseRepoGitURI, withWorkspaceRootInputRevision } from '../util/url.ts'

import type { HoverContext } from './HoverOverlay.tsx'

const LOADING = 'loading' as const

/**
 * This function is passed to {@link module:~/sg/codeintellify.createHoverifier}, which uses it to fetch
