...
import { isErrorLike, isFirefox, logger } from '~/internals/development/sg/common'

import type { SharedEventLogger } from '../../api/sharedEventLogger.ts'
import { type Event, EventClient, EventSource } from '../../graphql-operations.ts'
import type { UTMMarker } from '../../tracking/utm.ts'
import { EventName } from '../event-names.ts'
import type { TelemetryService } from '../telemetryService.ts'

import { logEvent } from './backend.ts'
import { observeQuerySelector } from './dom.ts'
import { sessionTracker } from './sessionTracker.ts'
import { userTracker } from './userTracker.ts'
import { stripURLParameters } from './util.ts'
...
export class EventLogger implements TelemetryService, SharedEventLogger {
    ...
    public logViewEvent(pageTitle: string, eventProperties?: any, logAsActiveUser = true): void {
        ...
        if ((window as any).context?.userAgentIsBot || !pageTitle) {
            return
        }
    ...
    public logPageView(eventName: string, eventProperties?: any, logAsActiveUser = true): void {
        ...
        if ((window as any).context?.userAgentIsBot || !eventName) {
            return
        }
    ...
    public log(eventLabel: string, eventProperties?: any, publicArgument?: any): void {
        ...
        if ((window as any).context?.userAgentIsBot || !eventLabel) {
            return
        }
    ...
    public getClient(): string {
        if ((window as any).context?.sourcegraphDotComMode) {
    ...
    private createEvent(event: string, eventProperties?: unknown, publicArgument?: unknown): Event {
        return {
            ...
            connectedSiteID: (window as any).context?.siteID,
            hashedLicenseKey: (window as any).context?.hashedLicenseKey,
        }
    }
}
