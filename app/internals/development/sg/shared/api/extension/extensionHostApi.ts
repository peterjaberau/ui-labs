...
} from '~/internals/development/sg/client-api'
import { LOADING, type MaybeLoadingResult } from '~/internals/development/sg/codeintellify'
import { combineLatestOrDefault, isDefined, isExactly, isNot, logger } from '~/internals/development/sg/common'
import type * as clientType from '~/internals/development/sg/extension-api-types'
import type { Context } from '~/internals/development/sg/template-parser'

import type { ReferenceContext, DocumentSelector } from '../../codeintel/legacy-extensions/api.ts'
import { getModeFromPath } from '../../languages.ts'
import { parseRepoGitURI } from '../../util/url.ts'
import { match } from '../client/types/textDocument.ts'
import type { FlatExtensionHostAPI } from '../contract.ts'
import type { ExtensionViewer, ViewerId, ViewerWithPartialModel } from '../viewerTypes.ts'

import { ExtensionCodeEditor } from './api/codeEditor.ts'
import { providerResultToObservable, proxySubscribable } from './api/common.ts'
import { computeContext, type ContributionScope } from './api/context/context.ts'
import {
    evaluateContributions,
    filterContributions,
    mergeContributions,
    parseContributionExpressions,
} from './api/contribution.ts'
import { ExtensionDirectoryViewer } from './api/directoryViewer.ts'
import { ExtensionDocument } from './api/textDocument.ts'
import { fromLocation, toPosition } from './api/types.ts'
import { ExtensionWorkspaceRoot } from './api/workspaceRoot.ts'
import { updateContext } from './extensionHost.ts'
import type { ExtensionHostState } from './extensionHostState.ts'
import { addWithRollback } from './util.ts'
