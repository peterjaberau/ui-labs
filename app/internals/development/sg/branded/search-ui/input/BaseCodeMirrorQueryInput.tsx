...
import { useCodeMirror, useCompartment } from "~/internals/development/sg/shared/components/CodeMirrorEditor.tsx"
...
import { type QueryInputEventHandlers, searchInputEventHandlers, setSearchInputEventHandlers } from "./codemirror/event-handlers.ts"
import { multiline, toSingleLine } from "./codemirror/multiline.ts"
import { parseInputAsQuery, setQueryParseOptions } from "./codemirror/parsedQuery.ts"
import { querySyntaxHighlighting } from "./codemirror/syntax-highlighting.ts"
...
export const BaseCodeMirrorQueryInput = memo(
  forwardRef<EditorView, BaseCodeMirrorQueryInputProps>(function BaseCodeMirrorQueryInput(
    ...
    const allExtensions = useMemo(
      () => [externalExtension, parsedQueryExtension, eventHandlers, multiLineExtension, readOnlyExtension, themeExtension, staticExtensions],
      [eventHandlers, parsedQueryExtension, readOnlyExtension, multiLineExtension, externalExtension, themeExtension],
    )
    ...
    return (
      <TraceSpanProvider name="CodeMirrorQueryInput">
        <div ref={containerRef} className={classNames(className, "test-query-input", "test-editor")} data-editor="codemirror6" />
      </TraceSpanProvider>
    )
  }),
)

function useQueryParser(editorRef: RefObject<EditorView>, patternType: SearchPatternType, interpretComments: boolean): Extension {
