...
import { createFilterSuggestions, PREDICATE_REGEX, regexInsertText, repositoryInsertText } from "~/internals/development/sg/shared/search/query/completion-utils.ts"
import { decorate, toDecoration, type DecoratedToken } from "~/internals/development/sg/shared/search/query/decoratedToken.ts"
import { FILTERS, filterTypeKeys, resolveFilter, type FilterType } from "~/internals/development/sg/shared/search/query/filters.ts"
import { getSuggestionQuery } from "~/internals/development/sg/shared/search/query/providers-utils.ts"
import { scanSearchQuery } from "~/internals/development/sg/shared/search/query/scanner.ts"
import type { Filter, Token } from "~/internals/development/sg/shared/search/query/token.ts"
import type { SearchMatch } from "~/internals/development/sg/shared/search/stream.ts"
import { createSVGIcon } from "~/internals/development/sg/shared/util/dom.ts"
import { toPrettyBlobURL } from "~/internals/development/sg/shared/util/url.ts"

import { formatRepositoryStarCount } from "../../util"

import { queryTokens } from "./parsedQuery.ts"
 ...
 */
type SuggestionSource<R, C extends SuggestionContext> = (context: C, tokens: Token[], tokenAtPosition?: Token) => R | null | Promise<R | null>
...
export function searchQueryAutocompletion(
  ...
  const override: CompletionSource[] = sources.map((source) => (context) => {
    ...
    return source({ position, onAbort: (listener) => context.addEventListener("abort", listener) }, query.tokens, token)
  })
    ...
    // This renders the completion icon
    {
      render(completion) {
        ...
        const icon = createSVGIcon(
          completion.type && completion.type in typeIconMap ? typeIconMap[completion.type as CompletionType] : typeIconMap[SymbolKind.UNKNOWN],
    ...
    sources.push(
      ...
      createDefaultSource((context, tokens, token) => {
        ...
        const suggestions = [...FILTER_SUGGESTIONS, ...FILTER_SHORTHAND_SUGGESTIONS].filter((suggestion) =>
          token?.type === "pattern" ? suggestion.label.toLowerCase().includes(token.value.toLowerCase()) : true,
        )
        return {
          from,
          options: suggestions,
      ...
      createFilterSource((_context, _tokens, token, resolvedFilter: any) => {
        ...
        return {
          ...
          options: resolvedFilter.definition.discreteValues(value, options.isSourcegraphDotCom).map(({ label, insertText, asSnippet }: any, index:any) => {
      ...
      createFilterSource(async (context, tokens, token, resolvedFilter: any) => {
        ...
        if (options.isSourcegraphDotCom && (!token.value || (token.value.type === "literal" && token.value.value === ""))) {
          return null
        }
        ...
        const filteredResults = results
          ...
          .flatMap((match) => completionFromSearchMatch(match, options, token, tokens, { filterValue: token.value?.value }))
          .filter(isDefined)
    ...
    sources.push(
      ...
      createDefaultSource(async (context, tokens, token) => {
        ...
        return {
          ...
          to: token.range.end,
          filter: false,
          options: results.flatMap((match) => completionFromSearchMatch(match, options, token, tokens, { isDefaultSource: true })).filter(isDefined),
        }
      }),
    )
  }
...
function completionFromSearchMatch(
  ...
  const hasNonActivePatternTokens = tokens.find((token) => token.type === "pattern" && !isEqual(token.range, activeToken.range)) !== undefined
  switch (match.type) {
    case "path": {
      return [
        {
    ...
    case "symbol": {
      return match.symbols.map((symbol) => ({
        ...
        apply: symbol.name + " ",
        detail: params?.isDefaultSource ? basename(match.path) : `${startCase(symbol.kind.toLowerCase())} | ${basename(match.path)}`,
        info: match.repository,
      }))
    }
    default: {
 ...
 */
function createFilterSource<R, C extends SuggestionContext>(source: FilterSuggestionSource<R, C>): SuggestionSource<R, C> {
  return (context, tokens, token) => {
...
const FILTER_SUGGESTIONS: Completion[] = createFilterSuggestions(Object.keys(FILTERS) as FilterType[]).map(({ label, insertText, detail }) => ({
  label,
  type: "queryfilter",
  apply: insertText,
  detail,
...
export const FILTER_SHORTHAND_SUGGESTIONS: Completion[] = filterTypeKeys.flatMap((filterType) => {
  ...
  const completions: any = FILTERS[filterType].discreteValues?.(undefined, false) ?? []
  return completions.map((completion: any) => {
    const insertText: any = `${filterType}:${completion.label} `.toLowerCase()
    return {
      label: insertText,
...
function isTokenInRange(token: { type: DecoratedToken["type"]; range: { start: number; end: number } }, position: number): boolean {
...
function voiceOverComma(): HTMLElement {
  ...
  element.append((document as any).createTextNode(","))
  return element
}
