...
import { getFileMatchUrl, getRepositoryUrl, getRevision, type SymbolMatch } from "~/internals/development/sg/shared/search/stream.ts"
import { isSettingsValid, type SettingsCascadeProps } from "~/internals/development/sg/shared/settings/settings.tsx"
...
import { codeCopiedEvent, V2CodyCopyPageTypes } from "~/internals/development/sg/shared/tracking/event-log-creators.ts"
...
import { CopyPathAction } from "./CopyPathAction.tsx"
import { RepoFileLink } from "./RepoFileLink.tsx"
...
export const SymbolSearchResult: React.FunctionComponent<SymbolSearchResultProps> = ({
  ...
  const title = (
    <span className="d-flex align-items-center">
      <RepoFileLink
        ...
        repoDisplayName={repoDisplayName ? `${repoDisplayName}${revisionDisplayName ? `@${revisionDisplayName}` : ""}` : undefined}
  ...
  const navigateToFile = useCallback(
    (event: KeyboardEvent<HTMLElement> | MouseEvent<HTMLElement>): void => navigateToCodeExcerpt(event, openInNewTab ?? false, navigate),
    ...
    if (hasBeenVisible) {
      ...
      subscription = fetchHighlightedFileLineRanges(
        ...
        .subscribe((res) => {
          ...
          telemetryService.log("search.latencies.frontend.code-load", { durationMs: endTime - startTime }, { durationMs: endTime - startTime })
        ...
        <div className={styles.symbols}>
          {result.symbols.map((symbol, i) => (
            <div
              ...
              className={classNames(styles.symbol, resultStyles.clickable, resultStyles.focusableBlock, resultStyles.horizontalDividerBetween)}
            ...
            >
              <div className="mr-2 flex-shrink-0">
                <SymbolKind
                  kind={symbol.kind}
                  symbolKindTags={isSettingsValid(settingsCascade) && settingsCascade.final.experimentalFeatures?.symbolKindTags}
                />
              </div>
