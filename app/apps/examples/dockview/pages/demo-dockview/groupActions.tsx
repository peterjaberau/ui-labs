import { DockviewApi, DockviewGroupLocation, DockviewGroupPanel } from "~/apps/dockview/view"
import * as React from "react"
import { Button, Card, Space } from "antd"
import { Group, ExternalLink, X, Eye, EyeOff, Maximize, Minimize } from "lucide-react"
...
const GroupAction = (props: { groupId: string; groups: string[]; api: DockviewApi; activeGroup?: string }) => {
  ...
  return (
    <Card size={"small"} title={props.groupId} onClick={onClick} style={{ backgroundColor: isActive ? "#e6f7ff" : "white" }}>
      <Space>
        <Button
          icon={<Group />}
          onClick={() => {
  ...
  return (
    <Space direction="vertical" style={{ width: '250px' }}>
      {props.groups.map((groupId, index) => (
        <GroupAction key={`${groupId}-${index}`} {...props} groupId={groupId} />
      ))}
    </Space>
  )
}
