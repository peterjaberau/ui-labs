...
import { stateMigrate } from "./migrate/stateMigrate.ts";
import { z } from "zod";
import { PERSISTED_SNAPSHOT_KEY } from "./constants.ts";
import { CallerSchema } from "./schemas.ts";
import {
  ActorKitInputProps,
...
import { assert, getCallerFromRequest } from "./utils.ts";
...
>({
    ...
    implements ActorServer<TMachine>
  {
    ...
    restorePersistedActor(persistedSnapshot: SnapshotFrom<TMachine>) {
      ...
      const migrations = stateMigrate.generateMigrations(
        machine,
        persistedSnapshot,
        input
      );
      const restoredSnapshot = stateMigrate.applyMigrations(
        persistedSnapshot,
        migrations
      );
